// Camping_bot firmware snapshot: ackerman_esp32_imu_v3
// Date: 2026-06-12
// Notes: ESP32 drive + MPU6050 IMU + UDP status heartbeat + buzzer tone output.
//
// Buzzer wiring:
//   VCC -> ESP32 3V3 first. If it only clicks, try 5V VCC only after checking
//          the module I/O pin is 3.3V-safe.
//   GND -> ESP32 GND / robot common GND
//   I/O -> GPIO13
//
// v2 used simple LOW/HIGH output for a low-trigger active buzzer.
// v3 outputs a 2400Hz tone so passive buzzer modules can also make sound.
...
const unsigned int BUZZER_TONE_HZ = 2400;
...
void setBuzzerOffLevel() {
  digitalWrite(BUZZER_PIN, BUZZER_LOW_TRIGGER ? HIGH : LOW);
}
...
void setBuzzer(bool enabled) {
  ...
  if (enabled) {
    if (BUZZER_USE_TONE) {
      ...
      digitalWrite(BUZZER_PIN, BUZZER_LOW_TRIGGER ? LOW : HIGH);
    }
  } else {
    noTone(BUZZER_PIN);
    setBuzzerOffLevel();
...
void handleCommandPacket(char* packetBuffer) {
  if (strncmp(packetBuffer, "BUZZER,", 7) == 0) {
    ...
    setBuzzer(value != 0);
    Serial.printf("buzzer=%d tone_hz=%u\n", buzzerOn ? 1 : 0, BUZZER_TONE_HZ);
    return;
  }
...
void setup() {
  ...
  Serial.println("\n--- [MCU 1] Ackermann drive + IMU + buzzer tone v3 boot ---");
